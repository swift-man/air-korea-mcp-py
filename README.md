...
- `get_air_quality_forecast`, `get_pm25_weekly_forecast`, `get_station_measurements`, `get_bad_khai_stations`, `get_sido_measurements`를 MCP 도구로 제공합니다.
- 응답에는 `items`뿐 아니라 `response_header`, `response_body`, `api_payload`가 함께 포함되어 Air Korea 원본 필드를 그대로 확인할 수 있습니다.
...
`get_sido_measurements`는 시도명뿐 아니라 고유하게 해석 가능한 하위 행정구역과 일부 접미사 생략 입력도 받을 수 있습니다.

- 직접 시도명: `서울`, `경기`
- 시도 별칭: `서울특별시`, `경기도`, `제주특별자치도`
- 고유 하위 지역: `우면동` -> `서울`
- 접미사 생략 입력: `우면` -> `서울`, `수내` -> `경기`
- 조합 입력: `서초구 우면동` -> `서울`
- 조합 입력 + 접미사 생략: `분당구 수내` -> `경기`

단, `삼성동`, `판교`, `중앙동`처럼 여러 지역에 걸쳐 중복되는 이름은 자동으로 하나를 고르지 않고, 가능한 시도와 다시 입력할 예시를 함께 안내합니다.
