...
class RestApiRuntimeConfig:
    ...
    def from_env(cls) -> "RestApiRuntimeConfig":
        ...
        port_raw = os.getenv("AIR_KOREA_REST_PORT", str(DEFAULT_REST_PORT)).strip()

        try:
            port = int(port_raw)
        except ValueError as exc:
            raise AirKoreaError("AIR_KOREA_REST_PORT must be an integer.") from exc

        if port < 1 or port > 65535:
            raise AirKoreaError("AIR_KOREA_REST_PORT must be between 1 and 65535.")
